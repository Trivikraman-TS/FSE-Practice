# ANALYSIS & N+1 EXPLANATION (Task 3 Steps 87 & 90)

# 1. IDENTIFYING THE N+1 PROBLEM:
#    - Query in Task 2 (Step 84) fetches all enrollment records:
#      `enrollments = session.query(Enrollment).all()`
#    - Since SQLAlchemy relationships default to lazy loading, fetching the
#      student name (`enrollment.student.first_name`) and course name
#      (`enrollment.course.course_name`) triggers a separate SELECT query
#      for each related student and course, per enrollment.
#    - With 4 enrollments:
#      - 1 Query to fetch all enrollments.
#      - 4 Queries to fetch student details.
#      - 4 Queries to fetch course details.
#      - Total: 9 queries executed (2N + 1 queries).
#    - If there were 10,000 enrollments, this would fire 20,001 queries.

# 2. RESOLUTION VIA EAGER LOADING:
#    - By using `joinedload`, SQLAlchemy issues an SQL INNER/LEFT JOIN to
#      eagerly fetch student and course details in a single query.
#      `session.query(Enrollment).options(joinedload(Enrollment.student), joinedload(Enrollment.course)).all()`
#    - Queries executed: 1 query total.

# 3. BONUS (Django ORM Equivalent):
#    - In Django ORM, the equivalent resolution to avoid N+1 query problem
#      when dealing with ForeignKey relationships is using `select_related()`:
